  ...
  <div class="!space-y-6">
    ...
    <v-container class="!p-0 mx-auto !shadow-none !bg-card !text-card-foreground !flex flex-col gap-6 !rounded-xl !border">
      <v-data-table
        ...
        item-value="name"
      >
        <template v-slot:item.reservation_id="{ item }">
          <td>
            <a
              href="#"
            ...
            >
              <Eye class="w-4 h-4" />
              <span>RES-{{ item.reservation_id }}</span>
            </a>
          </td>
        </template>
...
const fetchReservations = async () => {
   try {
     const response = await axios.get(`${ import.meta.env.VITE_API_URL}/getUserTransactionsAdmin.php` );
